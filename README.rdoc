This is Sermo's fork of Ruport. The only changes are:
- The rubuy19-compat branch is merged into master
- The version number is bumped to 1.7.2 with .sermo on the end
- Fixed a bug where the row names wasn't returned correctly for Pivot tables 
